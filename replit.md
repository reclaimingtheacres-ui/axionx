...
- **Background geocoding**: `backup_scheduler.py` runs geocode batches every 10 minutes (30 jobs per batch, newest first, 1.1s per geocode to respect Nominatim rate limits). Jobs that fail geocoding 3 times are skipped (`geocode_fail` column). Client-side geocoder sends specific visible job IDs for targeted geocoding, tracks sent IDs to avoid duplicate requests.
