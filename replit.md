...
- **Azure Oryx Startup Fix (permanent):** `oryx-manifest.toml` at repo root sets `CompressDestinationDir="false"`. `startup.sh` also patches the manifest with `sed` on every boot before gunicorn launches — self-healing even if Oryx regenerates it with `"true"`. Production DB at `/home/site/data/axion.db` (persistent mounted storage). App extracted to `/tmp/8deabc...` by Oryx runtime; `startup.sh` uses `$PWD` as `APP_ROOT` to handle this correctly.
- **Schema Sentinel Pattern:** `_schema_is_current()` gates `init_db()` via `_lazy_init()`. Every new column added via `add_column_if_missing` **must** also be checked in `_schema_is_current()` — otherwise production DBs that pass all existing checks skip `init_db()` entirely and never receive the new column. Defensive `job.keys()` access guards against any remaining window where a column is queried before migration runs.
- **Client Response Reset Workflow (Suspended Files):** When a client replies on a suspended/on-hold file, admin can click "Client Responded" to reset the stale timer. Stores `last_client_response_at` and `suspended_followup_due_at` (now + 10 days) on the job. File enters a 10-day grace period ("Awaiting Client Instructions"), then becomes eligible for a post-response follow-up email (`Urgent Further Instructions Required`), then a further 10-day window before auto-escalation to closure queue. The sidebar STATUS AGE section switches label to "CLIENT STATUS" and shows phase-specific badges (grace/follow-up sent/pending closure). `_CUR_ELIGIBLE_STATUSES` unchanged; post-response track runs before the 28-day stale gate. New `request_type` values: `client_response_reset`, `post_response_followup`.
- **Wise VIR Bug Fixes:** Three bugs fixed: (1) Pre-population/double-entry — `_rl_pdf_context` now prioritises saved `rec` values over `item` autofill for year/make/model/colour/registration/vin, so corrections made in the form persist on re-open instead of being overwritten; (2) Damage list — removed incorrect `any_damage == 'YES'` gate so damage details now print in the PDF; (3) DELIVERED TO / HELD AT overlap — name and address are drawn on separate lines, each constrained to 255pts wide from x=162, eliminating overflow into the HELD AT TOWING YARD column.
