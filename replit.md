...
- **GeoOp Description preservation**: Raw GeoOp description text is stored in `jobs.geoop_source_description` (immutable) AND as a `job_field_notes` record with `note_type='geoop_import'`. Structured fields (reg, VIN, arrears, costs, NMPD/next payment, regulation type, security details) are parsed via `parse_description()` and populated on both `jobs` and `job_items`. A `backfill_geoop_descriptions()` function retroactively applies this to all previously imported jobs without overwriting user edits (uses CASE WHEN blank-only semantics).
