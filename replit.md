...
- 3-step pipeline: Stage CSV → Review Diagnostics → Execute Import. Plus Step 5: Targeted Range Import (`import_staged_jobs_range()`) for inserting a specific reference_no range from staging with full dedup by geoop_job_id, display_ref, internal_job_number, and client_job_number. Route: `POST /admin/geoop-import/import-range`.
- **Source-to-client mapping model**: `geoop_source_client_map` maps source/lender names (parsed_client_name from GeoOp) to canonical names. `client_id` is NULL until manually assigned to a real AxionX client. **No clients are auto-created.** `populate_source_client_map()` only builds source→canonical entries. Import resolves `client_id` only via this map — no fuzzy `_match_client()` fallback.
- **Client review page** (`GET /admin/geoop-import/client-review`): Lists client records linked via `geoop_source_client_map` with job counts and source name variants.
...
- **Conversation management**: Filter toggle (All/Unread), Mark All Read button, Delete conversation. Desktop: hover-to-reveal × delete button per conversation. Mobile: swipe-left-to-reveal Delete button with scroll-vs-swipe gesture discrimination. Delete removes user from conversation; if last participant, fully cleans up messages/reads/conversation. API: `POST /messages/mark-all-read`, `DELETE /messages/<conv_id>`.
- **Auto-notification on job assignment**: When a job is assigned to an agent (via job creation, direct assignment, or status update), the system automatically sends an in-app message: "You have a new job to attend to in [suburb]." Suburb extracted from `job_address` using `_extract_suburb()` — skips street/unit parts, strips state/postcode, returns locality name. Also sends APNs push notification with `{type: "job_assigned", job_id: <id>}`. Notification only fires when the assigned agent actually changes (not on re-saves). Helper: `_notify_agent_job_assigned(agent_id, job_id, job_ref, job_address)`.
- **Push notification settings** (mobile): Settings page shows a "Notifications" card (iOS wrapper only) with Enable button. Requests push permission via `pushNotifications` WKScriptMessageHandler bridge, registers device token via `POST /m/api/device/register`. Shows status: Enabled (green), Blocked (red with iOS Settings guidance), or not available.
...
- **Database tables**: `route_plans` (route metadata, start/end points, mode, distance, duration, `assigned_agent_id`), `route_plan_stops` (ordered stops with distance/duration/coordinates).
- **Agent assignment**: Admin can assign saved routes to agents via dropdown on desktop. Server-side enforced: only admin/both can set `assigned_agent_id`, validated against active users. Assigned agents can load routes via API.
- **Mobile My Routes tab**: Mobile route planner opens on "My Routes" tab showing routes the user created or was assigned (with Open button). Loads saved stops/totals directly from API.
